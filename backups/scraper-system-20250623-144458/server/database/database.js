...
class DatabaseService {
    ...
    return builders.map(builder => {
      ...
      return {
        ...
        vanTypes: builder.van_types ? 
          ...
          ['Custom Van'],
        amenities: builder.amenities ? this.parseAmenities(builder.amenities) : ['Custom Build'],
        services: builder.services ? JSON.parse(builder.services) : ['Custom Builds'],
        certifications: [],
        socialMedia: (() => {
          if (!builder.social_media) return {};
          try {
            // If it's already an object, return it
            if (typeof builder.social_media === 'object') {
              return builder.social_media;
            }
            
            // If it's a string, parse it
            if (typeof builder.social_media === 'string') {
              return JSON.parse(builder.social_media);
            }
            
            return {};
          } catch (e) {
            console.error(`❌ Error parsing social media for ${builder.name}:`, e.message);
            return {};
          }
        })(),
        reviews: [],
  ...
  // Parse amenities - handle both JSON arrays and comma-separated strings
  parseAmenities(amenitiesData) {
    if (!amenitiesData) return ['Custom Build'];
    
    // If it starts with '[', it's likely JSON
    if (amenitiesData.trim().startsWith('[')) {
      try {
        return JSON.parse(amenitiesData);
      } catch (e) {
        console.log('Error parsing amenities JSON:', e.message);
        return ['Custom Build'];
      }
    } else {
      // It's a comma-separated string
      return amenitiesData.split(',').map(item => item.trim()).filter(item => item.length > 0);
    }
  }
